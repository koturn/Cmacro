...
#  ifdef __GNUC__
#    warning Please include <stdio.h> before "macro.h"!!
#  endif
#endif
...
#  ifdef __GNUC__
#    warning Please include <stdlib.h> before "macro.h"!!
#  endif
 ...
 * ([-Wunused-value])が出る。
 *   @code  #define WSI_SWAP(a, b)  ((a != b) && (a ^= b ^= a ^= b, 1))@endcode
 *
 * @see I_SWAP(a, b)
 * @see WI_SWAP(a, b)
 * @see WSI_SWAP(a, b)
 ...
 * ワーニングレベルを上げると、計算結果を用いていないという趣旨のワーニング
 * ([-Wunused-value])が出る。
 *   @code  #define WSF_SWAP(a, b)  ((a != b) && (a += b -= (a = b - a), 1))@endcode
 *
 * @see F_SWAP(a, b)
 * @see WF_SWAP(a, b)
 * @see WSF_SWAP(a, b)
...
/*!
 * @brief ワーニング([-Wsequence-point])が伴う SI_SWAP()関数マクロ
 * @see I_SWAP(a, b)
 * @see SI_SWAP(a, b)
 * @see WI_SWAP(a, b)
 */
...
/*!
 * @brief ワーニング([-Wsequence-point])が伴う SF_SWAP()関数マクロ
 * @see F_SWAP(a, b)
 * @see SF_SWAP(a, b)
 * @see WF_SWAP(a, b)
 */
 ...
 *
 * C++では、GNU拡張機能を用いない。<br>
 * gcc/C++0x対応コンパイラでのみコンパイル可能<br>
 * (ただし、C++でコンパイルできない例も存在する)<br>
 * トップレベルで用いることはできない
 * @code
 * // 無名関数を関数ポインタに渡す例
