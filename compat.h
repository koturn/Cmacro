...
#else
typedef int errno_t;
#define __DUMMY_ERRNO_OK__   ((errno_t) 0)  //!< ダミーのエラーナンバー(0はエラー無しの意味)
#define __DUMMY_ERRNO_ERR__  ((errno_t) 1)  //!< ダミーのエラーナンバー(1はエラー有りの意味)
...
// Windowsならば、(Visual C++ / MinGW-gcc, MinGW-g++)
